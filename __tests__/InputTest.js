...
import { CAR_VALIDATE } from "../src/constant/messages.js";
...
describe("자동차 입력 및 유효성검사", () => {
  ...
  //     "", // 빈 문자열
  //     "   ", // 공백만
  //   ])('자동차 이름이 비어 있으면 에러를 발생시킨다.: "%s"', async (input) => {
  //     mockQuestions([input]);
  //     const app = new App();
  //     await expect(app.run()).rejects.toThrow(ERROR_MESSAGES.EMPTY_NAME);
  //   });
  // });
  ...
  //     const app = new App();
  //     await expect(app.run()).rejects.toThrow(ERROR_MESSAGES.NOT_A_NUMBER);
  //   });

  //   test("시도 횟수가 정수가 아니면 에러를 발생시킨다.", async () => {
  //     mockQuestions(["pobi,woni", "5.5"]);
  //     const app = new App();
  //     await expect(app.run()).rejects.toThrow(ERROR_MESSAGES.INVALID_ROUND);
  //   });
